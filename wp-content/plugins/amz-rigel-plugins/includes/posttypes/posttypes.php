 ...
 */
	...
	$por_arr = apply_filters( 'portfolio_cpt_args', $por_arr );
...
function my_manage_portfolio_columns( $column, $post_id ) {
	...
	switch( $column ) {
		...
		case 'thumb' :
			//Get Porfolio Image
			$portfolio_images = rigel_get_meta_value( $post_id, '_amz_gallery', '' );
			$portfolio_images = ! empty( $portfolio_images ) ? json_decode( $portfolio_images ) : '';
			$image_id = ! empty( $portfolio_images ) ? $portfolio_images[0]->itemId : '';

			echo rigel_get_image_by_id( 75, 75, $image_id, 0, 0, 1 );
