<?php
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if( 'yes' == $slider ) {
				...
				$output .= '<div class="staff-container">'; //Staff Container
					...
					//Staff Image
					$thumb = rigel_featured_thumbnail( (int)$width, (int)$height, 0, 1, 1 );					
	...
	function rigel_portfolio($atts, $content = null){
		...
		$portfolio_category = !empty( $portfolio_category ) ? explode( ',', $portfolio_category ) : '';
		$portfolio_ids = !empty( $portfolio_id ) ? explode( ',', $portfolio_id ) : '';
					...
					$output .= '<ul id="filters" class="option-set clearfix">
						...
						if( 'id' == $insert_type ) {
							if( ! empty( $portfolio_ids ) ) {
								$term = array();
								foreach( $portfolio_ids as $key => $portfolio_id ) {
									$term_obj = get_the_terms( $portfolio_id,'pix_categories' );
									foreach( $term_obj as $key => $value ) {
										$term[$value->slug] = $value->name;
									}									
								}

								foreach( $term as $key => $val ) {
									$output .= '<li><a href="#" data-filter=".'. esc_attr( $key ) .'">'. esc_html( $val ) .'</a></li>';    
								}
							}							
							
						}
						else if( 'category' == $insert_type ) {
							foreach( $portfolio_category as $portfolio_cat ){ 
								$term = get_term_by( 'slug', $portfolio_cat, 'pix_categories' );
								$output .= '<li><a href="#" data-filter=".'. esc_attr( $term->slug ) .'">'. esc_html( $term->name ) .'</a></li>';    
							}
						}
						else {
							$terms = get_terms( 'pix_categories' ); 
		...
		//Query
		if( !empty( $portfolio_ids ) && 'id' == $insert_type ){
		...
		else if( !empty( $portfolio_category ) && 'category' == $insert_type ){
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();			
			...
			$portfolio_image_style = ( 'masonry' == $portfolio_style ) ? $portfolio_image_style : '';
			...
			if( 'yes' == $slider ) {
				...
				// onclick for iphone browser
				$output .= '<div class="portfolio-container" onclick="">'; //portfolio Container
	   ...
	   wp_reset_query();
		...
		if( 'yes' != $slider ){
			...
			if( $pagination != 'none' && 'id' != $insert_type ) {
   ...
   function rigel_icon_box($atts, $content= null){
		...
		//Checking Title Style
   		...
   		$output = '<div class="icon-box '. esc_attr( $icon_style ) .' '. esc_attr( $el_class ) .' '. esc_attr( $align ) .' '. esc_attr( $animate_class ) .'"'. $slideTransition .' '. $slideDuration .' '. $slideDelay .'>';
   			...
   			$output .= '<div class="icon-box-content">';
				...
				//Checking Title Style
				if( $title_font || !empty( $content_color ) ) {
				 	$content_style = ' style="';				 	
					$content_style .= RH()->amz_get_font_and_color_styles( $content_font, '', '', 0 );
				 	$content_style .= !empty( $content_color ) ? 'color: '. esc_attr( $content_color ) .'; ' : '';
				 	$content_style .= '"';
				 }
	...
	function rigel_title($atts, $content = null, $code){
		...
		$css_style .= !empty( $title_margin ) ? 'margin: '. esc_attr( $title_margin ) .'; ' : '';
		$css_style .= !empty( $custom_font_size ) ? 'font-size: '. esc_attr( $custom_font_size ) .'; ' : '';
		$css_style .= !empty( $custom_title_color ) ? 'color: '. esc_attr( $custom_title_color ) .'; ' : '';
		$css_style .= '"';
		...
		if( $sub_title == "yes" && !empty( $sub_title_text ) ){
			...
			if( '' != $sub_title_margin || '' != $custom_sub_title_color || $sub_title_font != "{'family':'none', 'variant':'none', 'size':''}" ) {
				...
				$sub_title_style .= !empty( $custom_sub_title_color ) ? 'color:'. esc_attr( $custom_sub_title_color ) .';"' : '';
				$sub_title_style .= '"';
			}
		...
		if ( $line == 'yes' ) {
			...
			//Checking Line Style
			$line_style = 'style="';
			$line_style .= !empty( $custom_line_color ) ? 'background: '. esc_attr( $custom_line_color ) .'; ' : '';
			$line_style .= '"';

			$line_border = '<span class="line" '. $line_style .'></span>';
		} 
	...
	function rigel_counter( $atts, $content = null ){
		...
		$number_css = $title_css = $icon_css = $icon_class = '';
		
		if( '' != $number_font_size || '' != $custom_number_color ) {
			$number_css = ' style="';
			$number_css .= !empty( $number_font_size ) ? 'font-size: '. esc_attr( $number_font_size ) .'; ' : '';
			$number_css .= !empty( $custom_number_color ) ? 'color: '. esc_attr( $custom_number_color ) .'; ' : '';
			$number_css .= '"';
		}

		if( '' != $text_font_size || '' != $custom_title_color ) {
			$title_css = ' style="';
			$title_css .= !empty( $text_font_size ) ? 'font-size: '. esc_attr( $text_font_size ) .'; ' : '';
			$title_css .= !empty( $custom_title_color ) ? 'color: '. esc_attr( $custom_title_color ) .'; ' : '';
			$title_css .= '"';
		}

		if( '' != $custom_icon_color ) {
			$icon_css = ' style="';
			$icon_css .= !empty( $custom_icon_color ) ? 'color: '. esc_attr( $custom_icon_color ) .'; ' : '';
			$icon_css .= '"';
		}
		...
		if( $icon == 'yes' && !empty( $icon_name ) ){
			$output .='<div class="icon-left"'. $icon_css .'><i class="'. esc_attr( $icon_name ) .'"></i></div>';
		}
			
		$output .= '<div class="counter-box">
					<span class="counter-value"'. $number_css .'>'. esc_html( $number ) .'</span>
					<span class="content"'. $title_css .'>'. esc_html( $text ) .'</span>
					</div>	
					</div>';
		return $output;
	}
