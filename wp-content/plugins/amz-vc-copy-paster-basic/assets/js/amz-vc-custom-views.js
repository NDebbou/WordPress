...
( function( $, window, AmzVcpTemplates, AmzVcpHTML ){
	...
	AmzVcp.Views.VcContent = Backbone.View.extend({
		...
		createColumn: function( parent_id, column_params ) {
			...
			column_params = _.isUndefined( column_params ) ? { width: "1/1" } : column_params;
			return vc.shortcodes.create({
				shortcode: "vc_column",
	...
	$('.amz_templates_name_filter_close').on('click', function(e) {
		e.preventDefault();

		$('.tab-group-wrap').removeClass('in-search');
		$('.tab-content, .sub-tab-content, .tab-sub-group-wrap, .amz-sub-tab-group, .amz-template-con').css('display', '');
		$( '#amz_templates_name_filter' ).val('');

	});

	var timeout;
	$( '#amz_templates_name_filter' ).on('keyup',  function(event) {

		if( $( '#amz_templates_name_filter' ).val() != '' ) {
			$( '#amz_templates_name_filter' ).addClass('filled');
		}
		else {
			$( '#amz_templates_name_filter' ).removeClass('filled');
		}

		var searchText = $('#amz_templates_name_filter').val().toLowerCase().replace(/ +(?= )/g,'');

		window.clearTimeout(timeout);
	    timeout = window.setTimeout(function(){
	    	if( $.trim( searchText ) ) {
	    		$('.tab-group-wrap').addClass('in-search');
	       		search(searchText);
	    	}
	    	else{
	    		$('.tab-group-wrap').removeClass('in-search');
	...
	function search(searchText) {
	    ...
	    $section.each(function( index ) {
	    	...
	    	if( $tags.length > 0 ) {
	    		...
	    		tag = $tags.toLowerCase().split(',');

	    		if( tag && tag.hasSubStringOf( searchText ) !== -1 ){

	    			$('.tab-content, .sub-tab-content').css('display', 'block');
	    			$('.amz-sub-tab-group').css('display', 'none');
