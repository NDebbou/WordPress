...
function rigel_display_header_elements( $elems, $header_pos = 'default-header-lang', $page_side = '' ){
	...
	if( $elems == 'tel' ){
		...
		echo '<div class="header-elem">';
			...
			$f_copyright_t = rigel_get_option_value( 'copyright_text', shortcode_exists( 'blog-link' ) ? sprintf( __( '<p>&copy; %s %s, All Rights Reserved.</p>', 'rigel' ), date( 'Y' ), do_shortcode( '[blog-link]' ) ) : sprintf( __( '<p>&copy; %s %s, All Rights Reserved.</p>', 'rigel' ), date( 'Y' ), get_bloginfo( 'name' ) ) );
